...
int main(int argc, char* argv[]) {
    ...
    // Subcommand: run
    auto cmd_run = app.add_subcommand("run", "Run some application");

    // Positional argument of subcommand run (required)
    std::string cmd_run_application = ".";
    cmd_run
        ->add_option("<APPLICATION>", cmd_run_application,
            "Application to be launched as daemon")
        ->required();

    // Optional argument of subcommand run
    auto cmd_run_flag_exec = false;
    cmd_run->add_flag("-e,--exec", cmd_run_flag_exec,
        "Run application in the current terminal.");

    std::string cmd_run_directory = ".";
    cmd_run->add_flag("-d,--directory", cmd_run_directory,
    ...
    auto cmd_path = app.add_subcommand("path", "Show content of $PATH environment variable");

    //----- Relaunch command settings -----------------//
    //.................................................//

    // Subcommand relaunch: Relaunch process given its PID (Process ID)
    auto cmd_relaunch = app.add_subcommand("relaunch-pid", "Relaunch a process, given its PID");

    int pid_to_relaunch;
    cmd_relaunch
        ->add_option("<PID>", pid_to_relaunch,
            "PID of application to be relaunched")
        ->required();
    ...
    if (*cmd_run) {
        ...
        std::cout << "  =>>                  Application  = " << cmd_run_application
                  << std::endl;
        std::cout << "  =>>          Run in terminal flag = " << cmd_run_flag_exec
                  << std::endl;
        std::cout << "  =>>     Process current directory = " << cmd_run_directory
                  << std::endl;
